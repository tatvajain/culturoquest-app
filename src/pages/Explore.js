...
const PathSeries = ({ title, stages, onSelect }) => {
    ...
    // THIS IS THE FIX: We'll add a loading state to prevent crashing.
    if (!gameData) {
        return <div className="text-center p-8">Loading course progress...</div>;
    }
    
    const { userProgress, getQuestionIdsForSaga, correctlyAnsweredQIDs } = gameData;
        ...
        <div className="mt-12">
            ...
            <div className="flex gap-12 overflow-x-auto pb-10 -mx-6 px-6">
                {stages.map((stage) => {
                    ...
                    const isCurrent = userProgress.currentStage === stage.id;
        ...
        <>
            <section>
                <div className="text-center">
                    ...
                    <p className="mt-2 text-lg text-slate-600">Unlock new courses as you master the content in each series.</p>
                </div>
